...
(define-library (linked-sorted-list)
  (export new from-scheme-list sorted-list? empty? full? length
          find! delete! add! peek find-ternary!
      ...
      (let ternary-search
        ...
        (if (<= left right)
            (let ((point-a (+ left (quotient (- right left) 3)))
              ...
              (cond
                ((==? key (vector-ref vect point-a))
                 (current! slst point-a))
                ((==? key (vector-ref vect point-b))
                 (current! slst point-b))
                ((<<? key (vector-ref vect point-a))
                 (ternary-search left (- point-a 1)))
                ((<<? key (vector-ref vect point-b))
                 (ternary-search (+ point-a 1) (- point-b 1)))
                (else
                 (ternary-search (+ point-b 1) right))))
            (current! slst -1)))
      slst)
